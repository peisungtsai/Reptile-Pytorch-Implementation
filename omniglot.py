...
def split_dataset(dataset, ratio_train=0.9):
    """
    ...
    num_train = int(len(all_data) * ratio_train)
...
class Character:
    ...
    def _read_image(self, path):
        ...
        with open(path, 'rb') as in_file:
            img = Image.open(in_file).convert('L').resize((28, 28)).rotate(self.rotation)
            self._cache[path] = transform_image(img)
            return self._cache[path]
